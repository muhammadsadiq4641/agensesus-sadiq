...
namespace CommmonUtils {
  ...
  export const makeNftId = (tokenAddress: string, tokenId: string): string => {
    return `${tokenAddress}-${tokenId}`;
  };
  export const deStructureNftId = (
    nftId: string
  ...
  //   export const imagePreview = (file: any): string => {
  ...
  export const countDropIdOccurrences = (
    arr: any[],
    dropId: string
  ): number => {
  ...
  export const countNFTsByNFTId = (array: any[], nftId: string): number => {
    let count = 0;
  ...
  export const calculateTotalFeesAndAmount = (signedOrder: any): string => {
    // Calculate the sum of amounts from the fees array
    const feesTotal = signedOrder.fees.reduce(
      (total: number, fee: any) => total + parseInt(fee.amount),
      0
    );
